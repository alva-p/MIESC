    ...
    def validate_finding(
        ...
        try:
            ...
            for mythril_finding in result.get("findings", []):
                # swc_id is normalized to None (not "") when Mythril emits an issue
                # without an SWC id, so .get(..., "") still yields None for a
                # present-but-None key — coerce with `or ""` before .replace.
